...
```{r, echo = FALSE}
pacman::p_load(readabs, ggtext, tidytable, scales, stringr, forcats, purrr, ggplot2, lubridate, tibble, zoo, deeptime)
...
# the main earnings spreadsheet is not downloading
...
ggsave(paste0("coal_output/images4paper/mining_money_legend.png"), height = 1.5, width = 2)
```
...
ggsave(paste0("coal_output/images4paper/taxable_income.png"))
...
ggsave("coal_output/images4paper/average_weekly_earnings.png")
...
ggsave(paste0("coal_output/images4paper/salary_mass.png"))
...
ggsave(paste0("coal_output/images4paper/workers_per_total.png"), multisector)
...
early_prod_gg <- early_prod_to_combine %>% 
  ...
  labs(x = "", y = "Tonnes", colour = "Coal type") +
...
ggsave("coal_output/images4paper/early_production.png", early_prod_gg)
...
combo_prodwork_gg <- combo_prod_worker %>% 
  ...
  scale_colour_manual(values = c("black", "grey40", "blue3", "#6dd1e1", "tan3"), 
                      na.translate = F) +
  scale_y_continuous(labels = unit_format(unit = "M", scale = 1e-03),
  ...
  geom_richtext(aes(x = 1920, y = 300000), colour = worker_colour, label = "coal<BR>workers", family = "serif", size = 3, label.color = NA, fill = NA) +
    geom_richtext(aes(x = 1955, y = 410000), colour = jcb_colour, label = "JCB<BR>mechanisation", family = "serif", size = 3, label.color = NA, fill = NA) +
  annotate("rect", 
           fill = jcb_colour, 
           alpha = 0.1,
           xmin = 1950, 
...
ggsave(paste0("coal_output/images4paper/workers_prod_exports.png"), combo_prodwork_gg)
...
ggsave(paste0("coal_output/images4paper/export_values.png"), top_exports_gg)
