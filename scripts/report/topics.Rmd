...
dtm_run_path <-  "coal_output/dtm/erss_hansard_run/"
...
ggsave(paste0("coal_output/", img_subfolder, "/discussion_level_parties.png"), discussion_parties_gg)
...
ggsave(paste0("coal_output/", img_subfolder, "/party_diffs.png"), party_gg)
...
party_diffs_cat_gg <- plot_differences(category)


ggsave(paste0("coal_output/", img_subfolder, "/party_diffs_cat.png"), party_diffs_cat_gg)
...
### Manually aggregated categories
...
six_cats_gg <- all_topic_scores_df %>%
  mutate(category = as.character(fct_collapse(category,
                                              `supply &\nworkers` = c("supply", "workers"),
                                 `trade,\neconomics &\npolitics` = c("economy", "trade", "politics")))) %>% 
  process_labs(category,
               c(interesting_labels, "other issues", "trade,\neconomics &\npolitics", "supply &\nworkers"),
               party_var = NULL,
               time_var = legislature) %>% 
  ggplot() +
  geom_line(aes(year, prop, colour = category)) +
  basic_add_ons +
  scale_colour_manual(values = c("supply &\nworkers" = "red",
                                 "trade,\neconomics &\npolitics" = "blue",
                                 "environment" = "darkgreen")) +
  coord_geo(dat = short_eras_df, 
            ylim = c(0, 0.6),
                              size = 3, 
                              pos = 'bottom',
                              height = unit(1, "lines"),
                              center_end_labels = T) +
  plot_gov_add_on +
  labs(y = "", colour = "Categories") +
    guides(
    color = guide_legend(byrow = TRUE)
  ) +
  theme(legend.spacing.y = unit(0.3, "cm"))
  
ggsave(paste0("coal_output/", img_subfolder, "/six_categories.png"), six_cats_gg)
...
for(subl in unique(label_plot$label)) {
  ...
  ggsave(paste0("coal_output/", img_subfolder, "/party_", subl, ".png"))
...
```{r}
  ...
  label_plot %>% 
    ...
    theme(text = element_text(size=16),
      ...
      labs(y = "") +
    guides(colour = "none") +
    coord_cartesian(xlim = c(1983, 2020))
