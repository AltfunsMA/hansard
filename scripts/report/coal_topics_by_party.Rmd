...
relative_discussion %>% 
  filter(weighted_paras > quantile(weighted_paras, 0.9)) %>% 
  distinct(year) %>% 
  pull() %>% 
  sort() %>% 
  cat(sep = ", ")
...
```{r}
discussion_parties_gg <- relative_discussion %>% 
...
ggsave("coal_output/images4paper/party_diffs.png", g)
...
for(subl in unique(label_plot$label)) {
  ...
  if(str_detect(subl, "taxes|budget|climate change|disasters|big business|farming|electorate|coalminers|infrastructure|carbon"))
