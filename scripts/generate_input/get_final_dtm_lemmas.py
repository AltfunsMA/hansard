...
#! /data/greyroads/energy_roadmap/env/bin/python 
...
data = pd.read_csv(main_topic + "_data/" + main_topic + "_full_downloaded.csv")
...
pkl.dump(nlpdocs, open(main_topic + "_data/input/spacydocs.pkl", 'wb+'))
...
pkl.dump(lemmas, open(main_topic + "_data/input/lemmas.pkl", 'wb+'))
# del nlpdocs
...
pkl.dump(dtm_docs, open(main_topic + "_data/input/dtm_docs.pkl", 'wb+'))
...
df.to_csv(open(main_topic + "_data/input/final_dtm_lemmas.df", 'w+'), index=False)

with open(main_topic + "_data/input/vocab.txt", 'w+') as vf:
